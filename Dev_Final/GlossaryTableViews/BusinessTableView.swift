...
class BusinessTableView: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let word = model.word(at: indexPath.row, shouldUpdateCurrentIndex: false) {
            cell.textLabel?.text = word[BusinessModel.key1]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
         ...
         performSegue(withIdentifier: "show1", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "show1" {
           
            let view = segue.destination as! BusinessDetailView
