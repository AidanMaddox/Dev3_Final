...
class OtherTableView: UITableViewController {
    ...
    var model = OtherFactorModel.shared
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "oID", for: indexPath)
        
        
        // Configure the cell...
        if let word = model.word(at: indexPath.row, shouldUpdateCurrentIndex: false) {
            cell.textLabel?.text = word[OtherFactorModel.key1]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "show3" {
            ...
            let view = segue.destination as! OtherDetailView
